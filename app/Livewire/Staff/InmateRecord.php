        ...
        return $table
            ...
            ->actions([
                ActionGroup::make([
                    ...
                    Action::make('view')->color('warning')->icon('heroicon-o-eye')->visible(fn($record) => $record->status != 'discharge')->url(fn($record): string => route('staff.inmates-information', ['id' => $record])),
                    ...
                    ])->modalHeading('')->modalWidth('7xl')->modalSubmitAction(false)->visible(fn($record) => $record->status != 'discharge'),
                    Action::make('decriptive_record')->label('Descriptive Record')->icon('heroicon-o-document-text')->url(fn($record) => route('staff.inmate-description', ['id' => $record->id]))->form([
                        ...
                        )->visible(fn($record) => $record->status != 'discharge'),
                    ...
                    Action::make('recommit')->label('Recommit')->color('success')->icon('heroicon-o-adjustments-vertical')->visible(fn($record) => $record->status == 'discharge')->action(
                        function ($record) {
                            $record->update(['status' => 'approved']);
                        }
                    ),
                    DeleteAction::make('delete')->visible(fn($record) => $record->status != 'discharge'),
                ]),
            ])
            ->bulkActions([
                // ...
